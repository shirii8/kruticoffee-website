          ...
          <div className="absolute bottom-4 right-4 z-20">
            {!cartItems[id] ? (
              ...
              <div className="flex items-center gap-3 bg-[#1a0f0b]/90 backdrop-blur-xl border border-[#b49e94]/30 p-1.5 rounded-full shadow-2xl">
                <img className="h-6 w-6 cursor-pointer hover:scale-110 transition-transform" onClick={() => removeFromCart(id)} src={removeIconRed} alt="minus" />
                <p className="text-[10px] font-black text-[#f4e3d8] min-w-[12px] text-center">{cartItems[id]}</p>
                <img className="h-6 w-6 cursor-pointer hover:scale-110 transition-transform" onClick={() => addToCart(id)} src={addIconGreen} alt="plus" />
              </div>
            )}
          </div>
        </div>
        ...
        <div className="p-6 flex-1 flex flex-col justify-between">
          <div>
            <div className="flex justify-between items-start mb-2">
              ...
              <p className="text-[#f4e3d8] font-serif italic text-xl leading-none">${price}</p>
            </div>
          ...
          <div className="mt-6 flex justify-between items-center opacity-30 group-hover:opacity-100 transition-opacity">
            <span className="text-[8px] tracking-[0.5em] text-[#b49e94] uppercase font-bold">BATCH.26</span>
